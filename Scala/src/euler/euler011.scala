...
object euler011 {
  def main(args: Array[String]) {
    ...
    val problem = pStr.map(a => a.map(b => b.toInt))

    val index: List[Int] = M.numbers(0).takeWhile(_ < 4).map(_.toInt).toList

    def getRight       (x: Int, y:Int): List[Int] = index.map(i => problem(x + i)(y)    )
    def getDown        (x: Int, y:Int): List[Int] = index.map(i => problem(x)    (y + i))
    def getDiagonalDown(x: Int, y:Int): List[Int] = index.map(i => problem(x + i)(y + i))
    def getDiagonalUp  (x: Int, y:Int): List[Int] = index.map(i => problem(x + i)(y - i))

    val maxValue = M.numbers(0).takeWhile(_ <= 16).map(_.toInt).map(x => List(
        M.numbers(0).takeWhile(_ <= 19).map(_.toInt).map(y => getRight(x, y)       .reduce((a,b) => a*b)).max,
        M.numbers(0).takeWhile(_ <= 19).map(_.toInt).map(y => getDown(y, x)        .reduce((a,b) => a*b)).max,
        M.numbers(0).takeWhile(_ <= 16).map(_.toInt).map(y => getDiagonalDown(x, y).reduce((a,b) => a*b)).max,
        M.numbers(3).takeWhile(_ <= 19).map(_.toInt).map(y => getDiagonalUp(x, y)  .reduce((a,b) => a*b)).max)
        .max
    ).max
